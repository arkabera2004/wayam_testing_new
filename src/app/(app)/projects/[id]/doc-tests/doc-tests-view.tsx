...
import type { DocScenario } from "@/lib/demo-data";

export type DocScenarioRow = DocScenario & { id: string };
export type DocInfo = { name: string; size: string; sections: number | null; parsedAtLabel: string };

/** File types the upload control advertises. */
const ACCEPTED = [".md", ".txt", ".pdf", ".docx", ".json", ".yaml", ".csv"];
...
export function DocTestsView({
  document,
  initialScenarios,
}: {
  document: DocInfo;
  initialScenarios: DocScenarioRow[];
}) {
  const { toast } = useToast();
  const [step, setStep] = useState(0);
  const [scenarios, setScenarios] = useState<DocScenarioRow[]>(initialScenarios);

  const chosen = scenarios.filter((s) => s.selected);
  const toggle = (id: string) => {
    // Optimistic, then persisted. On failure the row is put back so the UI
    // never claims a selection the database did not accept.
    const next = !scenarios.find((s) => s.id === id)?.selected;
    setScenarios((prev) => prev.map((s) => (s.id === id ? { ...s, selected: next } : s)));
    void fetch(`/api/doc-scenarios/${id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ selected: next }),
    }).then((res) => {
      if (!res.ok) setScenarios((prev) => prev.map((s) => (s.id === id ? { ...s, selected: !next } : s)));
    });
  };
      ...
      {step === 0 && (
        <Card title="Source document" subtitle="Markdown, plain text, PDF, Word, JSON, YAML or CSV">
          <div className="border-muted bg-raised/40 flex flex-col items-center gap-2 rounded-xl border border-dashed px-6 py-10 text-center">
            ...
            <p className="text-label-md text-primary mt-1">{document.name}</p>
            <p className="text-caption text-quaternary">
              {document.size} · {document.sections} sections · parsed {document.parsedAtLabel}
            </p>
