  ...
  return (
    <PageBody>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {integrationCards.map((integration) => (
          <div
            ...
            className={cn(
              "border-muted bg-container flex flex-col rounded-xl border p-4",
              integration.soon && "opacity-50",
            )}
          >
            <div className="flex items-start justify-between gap-2">
              <span className="bg-raised text-label-md text-secondary grid h-9 w-9 place-items-center rounded-lg">
            ...
            <p className="text-heading-sm text-primary mt-3.5">{integration.name}</p>
            <p className="text-body-md text-tertiary mt-1.5 flex-1">{integration.description}</p>
            ...
            <Button
              ...
              onClick={() =>
                toast({
                  tone: integration.connected ? "info" : "success",
                  title: integration.connected
                    ? `${integration.name} settings opened`
                    : `${integration.name} connected`,
                  body: integration.connected
                    ? "Adjust events and channels below."
                    : "Parikshan will start reporting here on the next run.",
                })
              }
            >
      ...
      <Card
        ...
        actions={
          <>
            <Chip tone="success">Quality gate: ON</Chip>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Slack" subtitle="Posting to #qa-alerts">
        ...
        <Card title="Jira" subtitle="Not connected">
          <div className="flex flex-col gap-3">
      ...
      <div>
        <h2 className="text-label-sm text-tertiary mb-3">Quality gate on a pull request</h2>
