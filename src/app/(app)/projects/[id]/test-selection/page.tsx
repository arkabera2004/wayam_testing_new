...
import { testSelection } from "@/lib/demo-data";
 ...
 * those. The original took a repo and a commit range; here the diff is the one
 * carried in the demo dataset.
 */
...
export default function TestSelectionPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const s = testSelection;
